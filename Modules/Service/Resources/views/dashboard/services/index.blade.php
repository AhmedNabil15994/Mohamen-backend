...
@section('title', __('service::dashboard.services.routes.index'))
@section('content')
  ...
  <div class="page-content">
    <div class="page-bar">
      <ul class="page-breadcrumb">
        ...
        <li>
          <a href="#">{{__('service::dashboard.services.routes.index')}}</a>
        </li>
      </ul>
    </div>
        ...
        <div class="portlet light bordered">
          ...
          <div class="table-toolbar">
            <div class="row">
              <div class="col-md-6">
                <div class="btn-group">
                  <a href="{{ url(route('dashboard.services.create')) }}"
            ...
            <table class="table table-striped table-bordered table-hover"
              ...
              <thead>
                <tr>
                  ...
                  <th>#</th>
                  <th>{{__('service::dashboard.services.datatable.title')}}</th>
                  <th>{{__('service::dashboard.services.datatable.image')}}</th>
                  <th>{{__('service::dashboard.services.datatable.status')}}</th>
                  <th>{{__('service::dashboard.services.datatable.created_at')}}</th>
                  <th>{{__('service::dashboard.services.datatable.options')}}</th>
                </tr>
              </thead>
            </table>
          </div>
          <div class="row">
            <div class="form-group">
              ...
              <button type="submit"
                ...
                onclick="deleteAllChecked('{{ url(route('dashboard.services.deletes')) }}')">
...
<script>
  function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('dashboard.services.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
...
responsivePriority:1,
              width: '13%',
              title: '{{__('service::dashboard.services.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("dashboard.services.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("dashboard.services.destroy", ":id") }}';
