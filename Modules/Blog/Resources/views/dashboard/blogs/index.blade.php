...
@section('title', __('blog::dashboard.blogs.routes.index'))
@section('content')
  ...
  <div class="page-content">
    <div class="page-bar">
      <ul class="page-breadcrumb">
        ...
        <li>
          <a href="#">{{__('blog::dashboard.blogs.routes.index')}}</a>
        </li>
      </ul>
    </div>
        ...
        <div class="portlet light bordered">
          ...
          <div class="table-toolbar">
            <div class="row">
              <div class="col-md-6">
                <div class="btn-group">
                  <a href="{{ url(route('dashboard.blogs.create')) }}"
            ...
            <table class="table table-striped table-bordered table-hover"
              ...
              <thead>
                <tr>
                  ...
                  <th>#</th>
                  <th>{{__('blog::dashboard.blogs.datatable.title')}}</th>
                  <th>{{__('blog::dashboard.blogs.datatable.image')}}</th>
                  <th>{{__('blog::dashboard.blogs.datatable.status')}}</th>
                  <th>{{__('blog::dashboard.blogs.datatable.created_at')}}</th>
                  <th>{{__('blog::dashboard.blogs.datatable.options')}}</th>
                </tr>
              </thead>
            </table>
          </div>
          <div class="row">
            <div class="form-group">
              ...
              <button type="submit"
                ...
                onclick="deleteAllChecked('{{ url(route('dashboard.blogs.deletes')) }}')">
...
<script>
  function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('dashboard.blogs.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
...
responsivePriority:1,
              width: '13%',
              title: '{{__('blog::dashboard.blogs.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("dashboard.blogs.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("dashboard.blogs.destroy", ":id") }}';
